"""Logs JSON estructurados y propagacion de x-correlation-id.

Portado de booking-svc/app/observability.py (design decision 7). Cada linea
de log es un objeto JSON con al menos: timestamp, level, service, event y
correlation_id. Cuando el request trae un JWT valido tambien se agrega
...
# No se usa en este checkpoint, pero se mantiene: el checkpoint de auth
# (grupo 4) lo necesita para dejar el user_id decodificado del JWT en los logs.
